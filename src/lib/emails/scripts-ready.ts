...
interface SendScriptsReadyEmailOptions {
  to: string;
  clientName: string;
  loginUrl: string;
  scriptCount?: number;
...
function renderScriptsReadyHtml(
  clientName: string,
  loginUrl: string,
  scriptCount?: number,
): string {
  ...
  const countLine = scriptCount && scriptCount > 0
    ? `<strong style="color:${OCEAN};font-weight:500;">${scriptCount} neue Skripte</strong> warten auf dich.`
          ...
          <tr>
            <td style="background:#ffffff;border-radius:20px;padding:80px 56px;">
              ...
              <p style="margin:0 0 56px 0;font-family:${FONT_STACK};font-size:15px;line-height:1.7;color:${OCEAN};font-weight:300;">
                Hi ${clientName.split(" ")[0]}, ${countLine}
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto 56px auto;">
                <tr>
                  <td style="border-radius:999px;background:${OCEAN};">
                    <a href="${loginUrl}" style="display:inline-block;padding:16px 44px;font-family:${FONT_STACK};font-size:15px;color:#ffffff;text-decoration:none;font-weight:500;">Skripte ansehen</a>
                  </td>
                </tr>
              </table>
              <p style="margin:0;font-family:${FONT_STACK};font-size:13px;line-height:1.7;color:${OCEAN};opacity:0.55;font-weight:300;">
                ...
                <a href="${loginUrl}" style="color:${OCEAN};opacity:0.75;word-break:break-all;">${loginUrl}</a>
              </p>
            </td>
          </tr>
          <tr>
...
export async function sendScriptsReadyEmail({
  to,
  clientName,
  loginUrl,
  scriptCount,
}: SendScriptsReadyEmailOptions): Promise<void> {
  const { error } = await getResend().emails.send({
    from: `${FROM_NAME} <${FROM_EMAIL}>`,
    to,
    subject: "Deine Skripte sind ready",
    html: renderScriptsReadyHtml(clientName, loginUrl, scriptCount),
  });
