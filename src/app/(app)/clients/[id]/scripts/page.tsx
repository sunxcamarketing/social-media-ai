...
export default function ClientScriptsPage() {
  ...
  const openEdit = (script: Script) => {
    setEditing(script);
    setEditForm(scriptToForm(script));
    setDialogOpen(true);
  };

  const openNew = () => {
    setEditing(null);
    setEditForm(EMPTY_SCRIPT_FORM);
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        title: editForm.title, pillar: editForm.pillar, contentType: editForm.contentType,
        format: editForm.format, status: editForm.status,
        hook: editForm.hook, body: editForm.body, cta: editForm.cta,
        textHook: editForm.textHook,
        visualHook: editForm.visualHook,
        bRoll: editForm.bRoll,
        shotList: editForm.shotList,
