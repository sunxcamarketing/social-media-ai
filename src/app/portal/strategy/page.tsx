        ...
        {pillars.length > 0 && (
          <div className="glass rounded-2xl p-4 sm:p-6">
            ...
            <div className="space-y-4">
              {pillars.map((p, i) => (
                <div key={i} className="border border-ocean/[0.06] rounded-xl p-3 sm:p-4">
                  <h3 className="text-sm font-medium text-ocean mb-2 break-words">{p.name}</h3>
                  {p.subtopics && p.subtopics.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {p.subtopics.map((st, j) => (
                        <span key={j} className="text-[11px] bg-ocean/5 text-ocean/70 px-2 py-0.5 rounded break-words">{st}</span>
                      ))}
                    </div>
                  )}
                  {p.videoIdeas && p.videoIdeas.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {p.videoIdeas.map((vi, j) => (
                        <p key={j} className="text-xs text-ocean/60 break-words">- {vi.title}</p>
                      ))}
                    </div>
                  )}
                </div>
