  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground ml-12">Be The Brand — Content Strategie Workbook</p>
      </div>
      ...
      <div className="glass-strong rounded-2xl border border-white/[0.08] p-6">
        ...
        <p className="mt-4 text-xs text-muted-foreground/70 leading-relaxed max-w-2xl">
      ...
      <div className="space-y-4">
        ...
        <div className="glass rounded-2xl border border-white/[0.08] p-5 space-y-4">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <Link className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/40" />
              <Input
                value={trainUrl}
                ...
                className="pl-9 rounded-xl glass border-white/[0.08] h-11 text-sm"
              />
            </div>
          ...
          {trainError && (
            <p className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2">{trainError}</p>
          )}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select value={trainType} onChange={(e) => setTrainType(e.target.value)}
                    className="mt-1.5 w-full h-10 rounded-xl glass border border-white/[0.08] bg-transparent px-3 text-sm text-foreground focus:outline-none focus:border-white/20">
                ...
                <div>
                  <Label className="text-xs text-muted-foreground">Format <span className="opacity-50">(select one or more)</span></Label>
                  <FormatPicker
                    value={trainFormat}
        ...
        {data.trainingExamples.length > 0 && (
          <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
            <div className="grid grid-cols-[1fr_160px_160px_32px_32px] gap-3 px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]">
              <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">URL</p>
              <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Content Type</p>
            ...
            <div className="divide-y divide-white/[0.04]">
              {data.trainingExamples.map((ex) => {
                const colorClass = TYPE_COLORS[ex.contentType] || "bg-white/[0.05] text-muted-foreground border-white/[0.08]";
                return (
                  <div key={ex.id} className="grid grid-cols-[1fr_160px_160px_32px_32px] gap-3 px-5 py-3 items-center hover:bg-white/[0.02] transition-colors">
                    <div className="min-w-0">
                      <a href={ex.url} target="_blank" rel="noopener noreferrer"
                        className="text-xs text-muted-foreground hover:text-foreground truncate block underline underline-offset-2 decoration-white/20">
                        {ex.url}
                      </a>
                      {ex.note && <p className="text-[11px] text-muted-foreground/50 mt-0.5 truncate">{ex.note}</p>}
                    </div>
                    <span className={`inline-flex items-center rounded-lg border px-2 py-1 text-[11px] font-medium ${colorClass}`}>{ex.contentType}</span>
                    <span className="text-xs text-muted-foreground">{ex.format}</span>
                    <button onClick={() => setEditExample({ ...ex })}
                      className="h-7 w-7 flex items-center justify-center rounded-lg text-muted-foreground/40 hover:text-muted-foreground hover:bg-white/[0.05] transition-colors">
                      <Pencil className="h-3 w-3" />
                    </button>
                    <button onClick={() => deleteExample(ex.id)}
                      className="h-7 w-7 flex items-center justify-center rounded-lg text-muted-foreground/40 hover:text-red-400 hover:bg-red-500/10 transition-colors">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">What purpose does each piece of content serve?</p>
          </div>
          <Button
            onClick={() => setTypeDialogOpen(true)}
            variant="ghost"
            size="sm"
            className="h-9 gap-1.5 rounded-xl px-3 text-xs border border-white/[0.08] text-muted-foreground hover:text-foreground hover:bg-white/[0.05]"
          >
        ...
        <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[200px_1fr_1fr_32px] gap-4 px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]">
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Name</p>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Goal</p>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Best For</p>
            <span />
          </div>
          ...
          <div className="divide-y divide-white/[0.04]">
            {allContentTypes.map((type) => {
              const colorClass = TYPE_COLORS[type.name] || "bg-white/[0.05] text-muted-foreground border-white/[0.08]";
              return (
                <div
                  key={type.id}
                  className="grid grid-cols-[200px_1fr_1fr_32px] gap-4 px-5 py-3.5 items-center hover:bg-white/[0.02] transition-colors"
                >
                  <div>
                    ...
                    {type.custom && (
                      <span className="ml-2 text-[10px] text-muted-foreground/40 font-medium uppercase tracking-wider">custom</span>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground leading-relaxed">{type.goal}</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">{type.bestFor}</p>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">How is the content produced and delivered?</p>
          </div>
          <Button
        ...
        <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[180px_1fr_1fr_160px_32px] gap-4 px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]">
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Format</p>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Description</p>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Best Content Type</p>
            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Platform</p>
            <span />
          </div>
          ...
          <div className="divide-y divide-white/[0.04]">
            {allFormats.map((format) => (
              <div
                key={format.id}
                className="grid grid-cols-[180px_1fr_1fr_160px_32px] gap-4 px-5 py-3.5 items-center hover:bg-white/[0.02] transition-colors"
              >
                <div>
                  <p className="text-sm font-medium">{format.name}</p>
                  {format.custom && (
                    <span className="text-[10px] text-muted-foreground/40 font-medium uppercase tracking-wider">custom</span>
                  )}
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed">{format.description}</p>
                <p className="text-xs text-muted-foreground leading-relaxed">{format.bestContentType}</p>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">How to apply the formula to your content calendar</p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="glass rounded-2xl border border-white/[0.06] p-5 space-y-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              ...
              Example: <em className="text-foreground/60">Finance Tips, Behind My Business, Client Results.</em>
            </p>
          </div>
          ...
          <div className="glass rounded-2xl border border-white/[0.06] p-5 space-y-3">
            <div className="flex items-center gap-2.5">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-green-500/20 border border-green-500/20">
        ...
        <div className="glass rounded-2xl border border-white/[0.06] p-5 space-y-4">
          <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Example Editorial Calendar</p>
          <div className="grid grid-cols-5 gap-3">
            {EXAMPLE_CALENDAR.map(({ day, type, format }) => {
              const colorClass = TYPE_COLORS[type] || "bg-white/[0.05] text-muted-foreground border-white/[0.08]";
              return (
                <div key={day} className="space-y-2">
                  <p className="text-[11px] font-semibold text-muted-foreground text-center">{day}</p>
          ...
          {editExample && (
            <div className="space-y-4 pt-2">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground/60 truncate">{editExample.url}</p>
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Content Type</Label>
      ...
      <Dialog open={formatDialogOpen} onOpenChange={(v) => { if (!v) setFormatDialogOpen(false); }}>
        <DialogContent className="max-w-md glass-strong rounded-2xl border-white/[0.08]">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
              <Label className="text-xs text-muted-foreground">Best Content Type</Label>
              <Input
