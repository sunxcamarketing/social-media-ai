...
const STATUS_OPTIONS = [
  { value: "idea", label: "Idea", color: "bg-blue-500/10 text-blue-400 border-blue-500/20" },
  { value: "in-progress", label: "In Progress", color: "bg-amber-500/10 text-amber-400 border-amber-500/20" },
  { value: "done", label: "Done", color: "bg-green-500/10 text-green-400 border-green-500/20" },
];

function statusColor(status: string) {
  return STATUS_OPTIONS.find((s) => s.value === status)?.color || "bg-white/[0.05] text-muted-foreground border-white/[0.06]";
}
    ...
    <div className="space-y-8">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Video concepts and content ideas for this client</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-lg glass-strong rounded-2xl border-white/[0.08]">
            ...
            <div className="space-y-4 pt-2">
              ...
              <div>
                <Label className="text-xs text-muted-foreground">Description / Concept</Label>
                <Textarea
                  value={form.description}
                  onChange={(e) => setForm({ ...form, description: e.target.value })}
                  placeholder="Describe the video concept, hook idea, key message…"
                  rows={4}
                  className="mt-1.5 rounded-xl glass border-white/[0.08] text-sm leading-relaxed"
                />
              </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Badge variant="secondary" className="ml-2 rounded-lg px-3 py-1.5 text-xs bg-white/[0.05] border border-white/[0.08]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((idea) => (
          <div key={idea.id} className="group glass rounded-2xl p-5 space-y-3 transition-all duration-300 hover:bg-white/[0.05] hover:border-white/[0.1]">
            ...
            {idea.createdAt && (
              <p className="text-[10px] text-muted-foreground/50">{idea.createdAt}</p>
            )}
          </div>
        ))}
        ...
        {filtered.length === 0 && (
          <div className="col-span-full glass rounded-2xl p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Add your first video concept to get started.</p>
          </div>
        )}
      </div>
    </div>
