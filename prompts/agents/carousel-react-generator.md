...
11. **Helper-Komponenten IMMER vollständig.** Jede `const Foo = (props) => (` MUSS einen JSX-Body und ein schließendes `);` haben. Niemals eine Helper-Deklaration starten ohne sie zu beenden: der Babel-Parser bricht sofort und der ganze Carousel rendert nicht. Bevor du die nächste `const`/`function` schreibst, prüf dass die vorige geschlossen ist. Bei langem Output: Helper kompakt halten, lieber Inline-JSX in den Slides als 8 verschachtelte Helper.
12. **TSX am Ende abschließen.** Output endet mit `}` der `function Carousel`-Funktion. Keine offenen Klammern, keine offenen JSX-Tags, kein "und so weiter…". Wenn du merkst dass dir der Platz ausgeht: kürze Inhalte (weniger Decoration, kürzere Texte), aber liefere niemals halben Code aus.
