// Preview send: ships the carousel-tutorial email to Aysun's inbox so
// she can confirm the layout/wording before forwarding to clients.
//
// Run: `node scripts/send-carousel-tutorial-preview.mjs`
//
// To swap in real screenshots later: edit the `screenshotUrl` field on
// each step in STEPS below (any public image URL works — Supabase
// Storage, Imgur, your CDN). Without URLs the email renders a clean
// "[Screenshot zu Schritt N]" placeholder bar.

import "dotenv/config";
import { Resend } from "resend";

const PREVIEW_TO = "caliskan2809@gmail.com";
...
const STEPS = [
  {
    ...
    screenshotUrl: `${SHOT}/step-1-sidebar.png`,
  },
  {
    title: "Thema eingeben",
    body: 'Im Eingabefeld unten: schreib in 1-2 Sätzen, was der Carousel vermitteln soll. Beispiel: „Warum 90% aller Content-Creator nach 6 Monaten aufhören — und wie AI das verhindert."',
    screenshotUrl: `${SHOT}/step-2-thema.png`,
  },
  {
    ...
    screenshotUrl: `${SHOT}/step-3-picker.png`,
  },
  {
    title: "Style Guide wählen",
    body: "Aus dem Dropdown oben den Style Guide auswählen — das fixiert Schriften, Farben und Layout-Look. Einmal aussuchen, alle deine Karussells sehen dann gleich aus.",
    screenshotUrl: `${SHOT}/step-4-styleguide.png`,
  },
  {
    ...
    screenshotUrl: `${SHOT}/step-5-starten.png`,
  },
  {
    title: "Im Chat anpassen",
    body: 'Du kannst unten im Chat schreiben was geändert werden soll: „Slide 2 kürzer", „mein Handle als Footer in alle Slides", oder zieh einfach ein Foto rein — dann wird es eingebaut.',
    screenshotUrl: `${SHOT}/step-6-chat.png`,
  },
  {
    title: "Als PNG exportieren",
    body: 'Oben rechts „Alle als PNG" klicken → du bekommst eine Datei pro Slide. Direkt zu Instagram hochladen, fertig.',
    screenshotUrl: `${SHOT}/step-7-export.png`,
  },
];
...
function renderStep(idx, step) {
  // Visual slot priority: screenshotUrl > placeholder.
  // The colspan="2" on the visual row is critical — without it the image
  // gets squeezed into the 48px number column on mobile clients.
  // We also force display:block + width:100% on the img to defeat
  // Gmail's intrinsic-size fallback when an image is wider than the
  // container's natural width.
  let visual;
  if (step.screenshotUrl) {
    visual = `<tr><td colspan="2" style="padding:18px 0 0 0;"><img src="${step.screenshotUrl}" alt="Schritt ${idx + 1}" width="544" style="display:block;width:100%;max-width:100%;height:auto;border-radius:12px;border:1px solid ${OCEAN}10;" /></td></tr>`;
  } else {
    visual = `<tr><td colspan="2" style="padding:18px 0 0 0;"><div style="background:${BLUSH_LIGHT};border:1px dashed ${BLUSH};border-radius:12px;padding:32px 16px;text-align:center;font-size:11px;color:${OCEAN}80;letter-spacing:0.1em;text-transform:uppercase;">[ Screenshot zu Schritt ${idx + 1} ]</div></td></tr>`;
  }
  return `<tr>
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="640" style="max-width:640px;">
        ...
        <tr><td style="background:#ffffff;border-radius:20px;padding:56px 48px;">
          <p style="margin:0 0 12px 0;font-size:13px;line-height:1.5;color:${OCEAN};font-weight:500;letter-spacing:0.18em;text-transform:uppercase;opacity:0.55;">Neues Tool für ${CLIENT_NAME}</p>
...
const { data, error } = await resend.emails.send({
  ...
  to: PREVIEW_TO,
  subject: "[Preview] Neu: Karussells selber bauen",
  html,
});
